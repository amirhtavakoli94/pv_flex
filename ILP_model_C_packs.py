...
def build_common_model(inst):
    ...
    for t in horizon:
        svar_BG[t, 1] = lp.addVar(vtype=GRB.BINARY)
        svar_GB[t, 1] = lp.addVar(vtype=GRB.BINARY)
        for i in range(1,7):
            svar_SG[t, i] = lp.addVar(vtype=GRB.BINARY)
            svar_GD[t, i] = lp.addVar(vtype=GRB.BINARY)
            
    lp.update()
    for t in horizon:
        lp.addConstr(gp.quicksum(svar_SG[t, i] for i in range(1,7)) == svar_G[t])
        
        lp.addConstr(gp.quicksum(svar_GD[t, i] for i in range(1,7)) == 1-svar_G[t])
        
        lp.addConstr(svar_BG[t, 1] <= 1-svar_B[t])
        
        lp.addConstr(svar_GB[t, 1] <= svar_B[t])
        
            
    for t in horizon:
        lp.addConstr(qvar_SG[t] <= gp.quicksum(100.1*i*svar_SG[t, i] for i in range(1, 7)))
        lp.addConstr(qvar_SG[t] >= gp.quicksum(99.9*i*svar_SG[t, i] for i in range(1, 7)))
        
#@        lp.addConstr(qvar_GD[t] <= gp.quicksum(100.1*i*svar_GD[t, i] for i in range(1, 7)))
#@        lp.addConstr(qvar_GD[t] >= gp.quicksum(99.9*i*svar_GD[t, i] for i in range(1, 7)))
        
        lp.addConstr(qvar_BG[t] == 100*svar_BG[t, 1])
        lp.addConstr(qvar_GB[t] == 100*svar_GB[t, 1])
